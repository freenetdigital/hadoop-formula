...
download-spark-archive:
  cmd.run:
    ...
    - user: {{ username }}
    - unless: test -f {{ spark.download_dir }}/spark-{{spark.version}}/VERSION
...
unpack-spark-archive:
  archive.extracted:
    - name: {{ spark.download_dir }}
    - source: file://{{ spark.download_dir }}/spark-{{ spark.version}}-{{spark.release}}.tgz
    - archive_format: tar
    - user: {{ username }}
    - group: {{ username }}
    - onchanges:
